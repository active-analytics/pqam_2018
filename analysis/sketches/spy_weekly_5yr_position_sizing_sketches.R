...
# reading-in data
df_managed_pnl <-
    read_csv("data_output/spy_weekly_2014_2018_managed_pnl_10_300.csv")
...
df_sit_out <- 
    read_csv("data_output/spy_weekly_2014_2018_chain_sit_out.csv")
...
dbl_commission <- 1.0
dbl_size_param <- 0.0035 # daily prem sold per dollar of AUM
...
# adding in implied_forward to df_scaling
df_scaling <- 
    df_scaling %>% 
        left_join(
...
# df_strategy_compy %>%
#     filter(trade_date >= lubridate::ymd(20140101)) %>% 
